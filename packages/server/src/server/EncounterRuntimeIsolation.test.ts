 ...
 *
 * Coverage strategy:
 * - ERI-001..005, 007..014: Reuse existing test coverage (see references below)
 * - ERI-005: New test proving the Legacy tickTimeouts runtime is never reached
 *   by GameLoop (ENABLE_BATTLE_COMBAT retired, New Combat is the sole path).
 *
 * Reference map:
...
describe("Phase 3I-3A — Encounter Runtime Isolation (ERI-001..015)", () => {
  ...
  it("ERI-005: Legacy tickTimeouts NEVER executed by GameLoop", () => {
    // With ENABLE_BATTLE_COMBAT retired, the GameLoop no longer calls the
    // Legacy Encounter runtime: tickEncounters/tickTimeouts were deleted and
    // tickCombatSessions is the sole tick source. All synchronous — no timers.
    ...
    // Minimal spawner: one idle mob with NO pendingEncounterTarget.
    ...
    // New Combat is the sole path — the Legacy runtime is never reached.
    loop.tick(1000);
    loop.tick(2000);
