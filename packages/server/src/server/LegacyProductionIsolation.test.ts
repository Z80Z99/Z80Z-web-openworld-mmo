...
describe("Phase 3I-2 — Legacy Production Isolation (LPI-001..015)", () => {
  ...
  it("LPI-005: flag=true → beginEncounter NOT called in tickMobAI", () => {
    ...
    // beginEncounter should NOT be called when flag is ON
    expect(spy).not.toHaveBeenCalled();
    // pendingEncounterTarget should be drained (not accumulated)
    expect(mob.pendingEncounterTarget).toBeNull();
  });

  it("LPI-006: flag=false → beginEncounter IS called in tickMobAI", () => {
    process.env.ENABLE_BATTLE_COMBAT = "false";
    ...
    // beginEncounter SHOULD be called when flag is OFF (Legacy path)
    expect(spy).toHaveBeenCalled();
  });

  it("LPI-007: flag=true → pendingEncounterTarget drained (not accumulated)", () => {
    process.env.ENABLE_BATTLE_COMBAT = "true";
    ...
    // Tick multiple times — pendingEncounterTarget should be drained each time
    loop.tick(1000);
    expect(mob.pendingEncounterTarget).toBeNull();

    // Simulate MobSpawner writing a new pendingEncounterTarget
    mob.pendingEncounterTarget = "player_2";
    loop.tick(2000);
