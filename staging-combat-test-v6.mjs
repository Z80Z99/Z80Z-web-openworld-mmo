/**
 * Phase 3G-4B.5: Combat Runtime Validation v6 (FINAL)
 *
 * ROOT CAUSE of v5 HP=0/0: Parser reads lines BACKWARDS from "回合",
 * so "60 / 60" is processed before "Scorpion (Lv.3)" — mobName is empty
 * when HP check runs → hpMatch fails the `mobName` guard → HP stays 0.
 *
 * FIX: Search FORWARD from "回合" line for mob name, then HP.
 * The encounter panel format is always:
 *   "MobName (Lv.N)"  →  "HP / MaxHP"  →  "回合 N"  →  "你的回合/对方行动中"
 */
...
/**
 * Read encounter state from body.innerText.
 *
 * Encounter panel format (lines appear in this order):
 *   "MobName (Lv.N)"
 *   "HP / MaxHP"
 *   "回合 N"
 *   "你的回合" or "对方行动中..."
 ...
 *   "逃跑"
 *
 * Strategy: Find "回合 N" line index, then look at lines BEFORE it
 * for mob name and HP. But search FORWARD through the preceding lines
...
async function readState(page) {
  return page.evaluate(() => {
    ...
    if (panelVisible) {
      // Search backwards from roundIdx, but track name and HP separately
      let foundName = false;
      for (let i = roundIdx - 1; i >= 0 && i >= roundIdx - 4; i--) {
        ...
        if (hpMatch && !mobName) {
          // HP line is closer to "回合" than the name line when searching backwards.
          // Accept it unconditionally — it's the encounter panel HP.
    ...
    // Parse turn
    let turn = 'unknown';
    if (panelVisible && roundIdx >= 0 && roundIdx + 1 < lines.length) {
      const turnLine = lines[roundIdx + 1];
      if (turnLine?.includes('你的回合')) turn = 'player';
      else if (turnLine?.includes('对方行动中')) turn = 'mob';
    }
...
async function testS1(p1) {
  ...
  record(`S1 pre-trigger: panel=${s0.panelVisible} mob=${s0.mobName} HP=${s0.mobHp}/${s0.mobMaxHp} turn=${s0.turn}`);

  if (!s0.panelVisible || s0.mobHp <= 0) {
    record('Triggering encounter...');
    await triggerEncounter(p1.page);
    s0 = await readState(p1.page);
    record(`S1 post-trigger: panel=${s0.panelVisible} mob=${s0.mobName} HP=${s0.mobHp}/${s0.mobMaxHp} turn=${s0.turn}`);
  }

  await p1.page.screenshot({ path: join(RESULTS_DIR, 's1-start.png') });

  if (!s0.panelVisible || !s0.mobName) {
    record('S1: FAIL — No encounter');
    return { passed: false, notes: 'No encounter' };
  }

  if (s0.mobHp <= 0) {
    record(`S1: Mob already dead (HP=${s0.mobHp}). Triggering fresh encounter...`);
    await triggerEncounter(p1.page);
    s0 = await readState(p1.page);
    record(`S1 retry: panel=${s0.panelVisible} mob=${s0.mobName} HP=${s0.mobHp}/${s0.mobMaxHp}`);
    if (s0.mobHp <= 0) {
      return { passed: false, notes: `Mob HP=0, no live mob available` };
    }
  }
  ...
  record(`S1 result: attacks=${attacks} killed=${killed} xp=${sf.xp}`);
...
async function testS2(p1, p2) {
  ...
  if (!p2s.panelVisible || p2s.mobHp <= 0) {
  ...
  if (!p2s.panelVisible || p2s.mobHp <= 0) {
    return { passed: false, notes: 'P2 no encounter / dead mob' };
  }
  ...
  return {
    ...
    notes: `P1: ${p1a} atk (XP=${f1.xp}), P2: ${p2a} atk (XP=${f2.xp})`
  };
}
...
async function testS8(p1) {
  ...
  if (!s0.panelVisible || s0.mobHp <= 0) {
    return { passed: false, notes: `No encounter or dead mob (HP=${s0.mobHp})` };
  }
  ...
  const killed = !sf.panelVisible || sf.mobHp <= 0;
  record(`S8 result: killed=${killed} attacks=${attacks} xp=${s0.xp}→${sf.xp}`);
...
// ============================================================
async function main() {
  record('Phase 3G-4B.5: Combat Runtime Validation v6 (FINAL)');
  record('='.repeat(60));
  ...
  writeFileSync(join(RESULTS_DIR, 'results-v6.json'), JSON.stringify({ timestamp: ts(), results, verdict }, null, 2));
  writeFileSync(join(RESULTS_DIR, 'validation-v6.log'), log.join('\n'));
